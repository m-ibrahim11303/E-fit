...
// Navigate to Exercises screen
    ...
    final button9 = find.byKey(Key('analytics_button'));
    expect(button9, findsOneWidget);
    await tester.tap(button9);
    await tester.pumpAndSettle();

    expect(find.textContaining('Analytics'), findsWidgets);

    final workoutHistoryButton = find.text('Workout\nHistory');
    await tester.tap(workoutHistoryButton);
    await tester.pumpAndSettle();
    ...
    final detailedAnalyticsButton = find.text('Detailed\nAnalytics');
    await tester.tap(detailedAnalyticsButton);
    await tester.pumpAndSettle();

    expect(find.textContaining('Activity Overview'), findsWidgets);
  });
}
